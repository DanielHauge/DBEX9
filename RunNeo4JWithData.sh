...
sudo docker run -d --name neo4j --rm --publish=7474:7474 --publish=7687:7687 --volume=$(pwd)/import:/import --volume=$(pwd)/plugins:/plugins --env NEO4J_dbms_memory_pagecache_size=6G --env NEO4J_dbms_memory_heap_max_size=10G --env NEO4J_AUTH=neo4j/class --env=NEO4J_dbms_security_procedures_unrestricted=apoc.\\\*,algo.\\\* neo4j
